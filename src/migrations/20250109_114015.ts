...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_light_primary_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_light_background_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_light_foreground_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_light_text_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_dark_primary_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_dark_background_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_dark_foreground_color\`;`)
  await db.run(sql`ALTER TABLE \`site_settings\` DROP COLUMN \`theme_dark_text_color\`;`)
}
